...
import { isValidEmail, sanitizeString, sanitizeHtml, validatePhone, validateFileSize, validateFileType } from '@/lib/validations'
...
// Rate limiting: 3 requests per 15 minutes per IP (more restrictive for applications)
const RATE_LIMIT_REQUESTS = 3
const RATE_LIMIT_WINDOW = 15 * 60 * 1000 // 15 minutes

// File upload limits
const MAX_FILE_SIZE_MB = 10
const ALLOWED_FILE_TYPES = ['pdf', 'doc', 'docx', 'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || !email || !position) {
      return errorResponse('Name, email, and position are required', 400)
    }
    ...
    if (position.length > 200) {
      return errorResponse('Position must be less than 200 characters', 400)
    }
    ...
    if (resume && resume.size > 0) {
      if (!validateFileSize(resume, MAX_FILE_SIZE_MB)) {
        return errorResponse(`Resume file size must be less than ${MAX_FILE_SIZE_MB}MB`, 400)
      }
    ...
    const sanitizedPosition = sanitizeString(position, 200)
    const sanitizedExperience = experience ? sanitizeString(experience, 50) : ''
    const sanitizedMessage = message ? sanitizeHtml(message, 5000) : ''
    ...
    if (resume && resume.size > 0) {
      try {
        ...
        // Sanitize filename
        const sanitizedFilename = sanitizeString(resume.name || 'resume.pdf', 255)
        ...
        return errorResponse('Error processing resume file', 400)
      }
    }
    ...
    const { data, error } = await resendClient.emails.send({
      ...
      subject: `New Job Application: ${sanitizedPosition} - ${sanitizedName}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); overflow: hidden;">
                  ...
                  <tr>
                    <td style="padding: 40px 30px;">
                      <!-- Company Info -->
                      <div style="text-align: center; margin-bottom: 30px; padding-bottom: 25px; border-bottom: 2px solid #f0f0f0;">
                        <p style="margin: 0; color: #6b7280; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                          <tr>
                            <td style="padding: 8px 0;">
                              <strong style="color: #374151; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 4px;">Applicant Name</strong>
                          ...
                          <tr>
                            <td style="padding: 8px 0;">
                              <strong style="color: #374151; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 4px;">Position Applied For</strong>
                              <span style="color: #1f2937; font-size: 15px; font-weight: 500;">${sanitizedPosition}</span>
                            </td>
                          </tr>
                          ${sanitizedExperience ? `
                          <tr>
                            <td style="padding: 8px 0;">
                              <strong style="color: #374151; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 4px;">Years of Experience</strong>
                              <span style="color: #1f2937; font-size: 15px; font-weight: 500;">${sanitizedExperience}</span>
                            </td>
                          </tr>
                          ` : ''}
                          ${resume ? `
                          <tr>
                            <td style="padding: 8px 0;">
                              <strong style="color: #374151; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px; display: block; margin-bottom: 4px;">Resume</strong>
                              <span style="color: #1f2937; font-size: 15px; font-weight: 500;">${sanitizeString(resume.name, 255)} (attached)</span>
                            </td>
                          </tr>
                          ` : ''}
                        </table>
                      ...
                      <div style="margin-bottom: 30px;">
                        <h3 style="margin: 0 0 15px 0; color: #1f2937; font-size: 16px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">Cover Letter / Additional Information</h3>
                      ...
                      <div style="padding-top: 25px; border-top: 1px solid #e5e7eb; text-align: center;">
                        <p style="margin: 0 0 8px 0; color: #6b7280; font-size: 12px; line-height: 1.6;">
                          This email was sent from the job application form on your website.<br>
...
Applicant Name: ${sanitizedName}
Email: ${sanitizedEmail}
${sanitizedPhone ? `Phone: ${sanitizedPhone}` : ''}
Position Applied For: ${sanitizedPosition}
${sanitizedExperience ? `Years of Experience: ${sanitizedExperience}` : ''}
${resume ? `Resume: ${sanitizeString(resume.name, 255)} (attached)` : ''}
...
Careers Email: careers@31stengineering.co.uk
    ...
    if (error) {
      ...
      return errorResponse('Failed to send application. Please try again later.', 500)
    }

    return successResponse(data, 'Application submitted successfully')
  } catch (error) {
